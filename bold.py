...
dwi_path = f"/media/joao-oliveira/PortableSSD/dynDWI_V2/sub027/BOLD/preproc/Glymphatics_mc_topup.nii.gz"
gm_path = f"/media/joao-oliveira/PortableSSD/dynDWI_V2/sub027/BOLD/preproc/GM_func.nii.gz"
dwi = nib.load(dwi_path).get_fdata()
gm = nib.load(gm_path).get_fdata()
df = pd.read_csv('/media/joao-oliveira/PortableSSD/dynDWI_V2/sub027/BOLD/bold.csv')
tempo_difusao = df[df['PEgy']==1]['Time']
...
tempo_difusao = (df[df["PEgy"] == 1]["Time"].values[:60])

#plot_slice_group_signals(dwi,gm_mask, tempo_difusao,TR=0.4,slice_scan_order="hf")

t_hyper, s_hyper = plot_hypersampled_curve(dwi, gm_mask,tempo_difusao,TR=0.4, slice_scan_order="hf")
